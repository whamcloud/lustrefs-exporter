...
use lustre_collector::quota::{parse as combine_parse, w_parse as winnow_parse};
...
async fn test_winnow_with_mem(buffer: &String) -> MemoryUsage {
    ...
    let needle = &mut buffer.as_str();
    winnow_parse.parse(needle).unwrap();
...
async fn test_combine_with_mem(buffer: &String) -> MemoryUsage {
    let (start_rss, start_virtual) = get_memory_stats();
    let mut rss_values = vec![];
    let mut virtual_values = vec![];
...
pub fn combine_memory(c: &mut Criterion) {
    ...
    if !samples.is_empty() {
        ...
        std::fs::write(output, serialized_metrics)
            .expect(&format!("Failed to write benchmark results to `{output}`"));

        println!("✅ Bencher results written to {output}");
    }
}

pub fn winnow_memory(c: &mut Criterion) {
    let (tx, rx) = std::sync::mpsc::channel::<MemoryUsage>();
    ...
    group.bench_function("winnow_memory", |b| {
        let mut raw = String::new();
        File::open("benches/quotas.yml")
            .unwrap()
            .read_to_string(&mut raw)
        ...
        b.to_async(&rt).iter(|| async {
            let memory_usage = test_winnow_with_mem(&raw).await;
    ...
    if !samples.is_empty() {
        ...
        let output = "winnow_mem_usage.json";
        std::fs::write(output, serialized_metrics)
            .expect(&format!("Failed to write benchmark results to `{output}`"));
...
pub fn winnow_perf(c: &mut Criterion) {
    let mut group = c.benchmark_group("parse_benchmarks");

    group.sample_size(10);
    group.measurement_time(Duration::from_secs(60)); // Allow more time

    group.bench_function("winnow_performance", |b| {
        let mut raw = String::new();
        File::open("benches/quotas.yml")
            .unwrap()
            .read_to_string(&mut raw)
