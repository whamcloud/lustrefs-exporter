...
fn test_stats_otel() {
    ...
    let (provider, registry) = init_opentelemetry().expect("Failed to initialize OpenTelemetry");
    ...
    let opentelemetry = read_metrics_from_snapshot(
        format!(
            "{}/tests/snapshots/r#mod__stats_otel.snap",
    ...
    let previous_implementation = read_metrics_from_snapshot(
        format!(
            "{}/tests/snapshots/r#mod__stats.snap",
            env!("CARGO_MANIFEST_DIR")
        )
        .as_str(),
    )
...
fn test_lnetctl_stats_otel() {
    ...
    let opentelemetry = read_metrics_from_snapshot(
        format!(
            "{}/tests/snapshots/r#mod__lnetctl_stats_otel.snap",
            env!("CARGO_MANIFEST_DIR")
        )
        .as_str(),
    );
    let previous_implementation = read_metrics_from_snapshot(
        format!(
            "{}/tests/snapshots/r#mod__lnetctl_stats.snap",
            env!("CARGO_MANIFEST_DIR")
        )
        .as_str(),
    );
...
fn test_lnetctl_stats_mds_otel() {
    ...
    let opentelemetry = read_metrics_from_snapshot(
        format!(
            "{}/tests/snapshots/r#mod__lnetctl_stats_mds_otel.snap",
            env!("CARGO_MANIFEST_DIR")
        )
        .as_str(),
    );
    let previous_implementation = read_metrics_from_snapshot(
        format!(
            "{}/tests/snapshots/r#mod__lnetctl_stats_mds.snap",
            env!("CARGO_MANIFEST_DIR")
        )
        .as_str(),
    );
...
fn test_host_stats_non_healthy_otel() {
    ...
    let opentelemetry = read_metrics_from_snapshot(
        format!(
            "{}/tests/snapshots/r#mod__host_stats_non_healthy_otel.snap",
            env!("CARGO_MANIFEST_DIR")
        )
        .as_str(),
    );
    let previous_implementation = read_metrics_from_snapshot(
        format!(
            "{}/tests/snapshots/r#mod__host_stats_non_healthy.snap",
            env!("CARGO_MANIFEST_DIR")
        )
        .as_str(),
    );
...
fn test_client_stats_otel() {
    ...
    let opentelemetry = read_metrics_from_snapshot(
        format!(
            "{}/tests/snapshots/r#mod__client_stats_otel.snap",
            env!("CARGO_MANIFEST_DIR")
        )
        .as_str(),
    );
    let previous_implementation = read_metrics_from_snapshot(
        format!(
            "{}/tests/snapshots/r#mod__client_stats.snap",
            env!("CARGO_MANIFEST_DIR")
        )
        .as_str(),
    );
    ...
    for dir in VALID_FIXTURES.find("*").unwrap() {
        match dir {
            ...
            include_dir::DirEntry::File(file) => {
                ...
                let previous_implementation = read_metrics_from_snapshot(
                    format!(
                        "{}/tests/snapshots/r#mod__valid_fixture_{name}.snap",
                        env!("CARGO_MANIFEST_DIR")
                    )
                    .as_str(),
                );
